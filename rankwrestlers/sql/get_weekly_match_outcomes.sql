...
from	(
		select	WinnerID = Winner.EventWrestlerID
				...
				, Division = case 
					...
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like '% middle%' then 'MS'
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like '% ms %' then 'MS'
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like '%/ms %' then 'MS'
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like '% ms/%' then 'MS'
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like '% jv %' then 'JV'
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like '% jv/%' then 'JV'
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like '%/jv%' then 'JV'
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like '% jv%' then 'JV'
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like 'jv %' then 'JV'
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like '%girl%' then 'Girls'
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like '%women%' then 'Girls'
					when nullif(trim(EventMatch.Division), '') is null and Event.EventName like '%woman%' then 'Girls'
					else 'HS'
					end
